...
struct TruthAnalysis
{
    ...
    TruthAnalysis(Arbusto &arbusto_ref, Nano &nt_ref, HEPCLI &cli_ref, Cutflow &cutflow_ref) : arbusto(arbusto_ref), nt(nt_ref), cli(cli_ref), cutflow(cutflow_ref) { std::cout << "Initialize Truth Analysis object" << std::endl; };
    ...
    void setTruthCandidates() // Nano &nt, Arbusto &arbusto, HEPCLI &cli)
    {
        ...
        {
            std::sort(hdecay_idx.begin(), hdecay_idx.end(), [this](int a, int b) { return nt.GenPart_pt().at(a) > nt.GenPart_pt().at(b); });
        }
        ...
        for (size_t iV = 0; iV < firstVs_idx.size(); ++iV)
        {
            ...
            {
                std::sort(vdecays_idx.begin(), vdecays_idx.end(), [this](int a, int b) { return nt.GenPart_pt().at(a) > nt.GenPart_pt().at(b); });
            }
        ...
        std::sort(v_infos.begin(), v_infos.end(),
                  [](const VInfo &a, const VInfo &b)
                  {
        ...
        if (vbsquarks_idx.size() == 2)
        {
            ...
            std::sort(vbsquarks_idx.begin(), vbsquarks_idx.end(), [this](int a, int b) { return nt.GenPart_pt().at(a) > nt.GenPart_pt().at(b); });
        }
    ...
    void printTruthInfo(Nano &nt, int igen)
    {
        ...
        std::cout << "    PtEtaPhiM:(" << std::setw(8) << nt.GenPart_pt().at(igen) << ", " << std::setw(8) << nt.GenPart_eta().at(igen) << ", " << std::setw(8) << nt.GenPart_phi().at(igen) << ", " << std::setw(8) << nt.GenPart_mass().at(igen) << ")";
    ...
    bool isQuark(int pdgId) { return (abs(pdgId) == 1) || (abs(pdgId) == 2) || (abs(pdgId) == 3) || (abs(pdgId) == 4) || (abs(pdgId) == 5) || (abs(pdgId) == 6); };
};
